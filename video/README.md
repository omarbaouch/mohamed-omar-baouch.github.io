...
Playwright et encode avec ffmpeg. Les scènes « Titre » et « Publication » reprennent les images
du film du hero (`public/film/hero/desk/`, à rendre d'abord) ; la bande-son est synthétisée par
`soundtrack.py` (120 BPM, chaque coupe tombe sur un temps — aucun échantillon externe).

| Vue | Temps | Contenu |
| --- | --- | --- |
| 01 Ouverture | 0 – 4,5 s | la mise en plan s'ouvre, chargement de ASM-BAOUCH.SLDASM |
| 02 Titre | 4,5 – 11 s | le hero ligne par ligne, sur la tempête de fichiers |
...
| 09 Publication | 46 – 53 s | tout converge vers la source unique, baouch.fr — ÉTAT PUBLIÉ |

## Régénérer

```sh
npm ci && npx vite build && npx vite preview --port 4174 &   # le site, pour les captures
node video/capture.mjs http://localhost:4174                 # → video/shots/ (non versionné, en mouvement réduit)
